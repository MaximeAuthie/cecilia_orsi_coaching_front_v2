describe('template spec', () => {
  ...
  it('passes', () => {
    cy.visit('https://www.maximeauthie.fr/api/user/logIn/' + env('valid_user') + '/' + env('valid_token'));
  ...
  it('valid-user-invalid-token', () => {
    cy.visit('https://www.maximeauthie.fr/api/user/logIn/' + env('valid_user') + '/' + env('invalid_token'));
  ...
  it('valid-user-expired-token', () => {
    cy.visit('https://www.maximeauthie.fr/api/user/logIn/' + env('valid_user') + '/' + env('expired_token'));
  ...
  it('invalid-user-valid-token', () => {
    cy.visit('https://www.maximeauthie.fr/api/user/logIn/' + env('invalid_user') + '/' + env('valid_token'));
    cy.get('.admin_content_filters_message_error').should('contain', 'Une erreur est survenue. Merci de vous reconnecter si vous souhaitez accéder à nouveau à l\'espace d\'administration.');
  })

  //? Test avec un utilisateur invalide et un jwt invalide
  it('invalid-user-invalid-token', () => {
    cy.visit('https://www.maximeauthie.fr/api/user/logIn/' + env('invalid_user') + '/' + env('invalid_token'));
    cy.get('.admin_content_filters_message_error').should('contain', 'Une erreur est survenue. Merci de vous reconnecter si vous souhaitez accéder à nouveau à l\'espace d\'administration.');
  })

  //? Test avec un utilisateur invalide et un jwt valide mais expiré
  it('invalid-user-expired-token', () => {
    cy.visit('https://www.maximeauthie.fr/api/user/logIn/' + env('invalid_user') + '/' + env('expired_token'));
    cy.get('.admin_content_filters_message_error').should('contain', 'Une erreur est survenue. Merci de vous reconnecter si vous souhaitez accéder à nouveau à l\'espace d\'administration.');
  })
})